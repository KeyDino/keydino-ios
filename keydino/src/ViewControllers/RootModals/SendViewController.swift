...
// File Description:
// - ViewController for sending a tx
        ...
        if sender.transaction == nil {
            ...
            if let minOutput = walletManager.wallet?.minOutputAmount {
                guard amount.rawValue >= minOutput else {
                    ...
                    let message = String(format: S.PaymentProtocol.Errors.smallPayment, minOutputAmount.string(isBchSwapped: store.state.isBchSwapped))
